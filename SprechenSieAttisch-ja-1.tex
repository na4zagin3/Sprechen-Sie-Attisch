...
\section*{文字について}
本書はギリシャ語部分にTheano Old Styleを用い、以下のOpenType Featureを適用しています。
\begin{itemize}
\item Common Ligatures
\item Contextural Ligatures
\end{itemize}
...
\item \textgreek{{\addfontfeature{Ligatures={NoCommon,NoContextual}}ου}}は\textgreek{ου}になります。
\item \textgreek{{\addfontfeature{Ligatures={NoCommon,NoContextual}}στ}}は\textgreek{στ}になります。
\end{itemize}
...
を参考にしましたが、全面的に依拠している訳ではありません（腑に落ちない点があるのでメールを送ってみようと思っています）。
