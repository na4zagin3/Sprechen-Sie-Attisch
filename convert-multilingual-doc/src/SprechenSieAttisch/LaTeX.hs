...
modernSentenceForHeader :: ModernSentence -> Document
modernSentenceForHeader ms = concat ["{", ss, "}"]
  where
    ss = concat $ map snd $ modernSentence ms
...
renderLexiconSection s = (docTitle, docConvs)
  where
    docTitle = subSectionStarHeader Nothing . modernSentenceForHeader <$> section s
...
convertLexicon p = L.intercalate "\n" [docTitle, docSections]
  where
    ...
    it = modernSentenceForHeader <$> Lexicon.indexTitle p
    t = modernSentenceForHeader $ lexicon p
