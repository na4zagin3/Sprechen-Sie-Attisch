...
\ifpdf
  \ifluatex
    ...
    \def\cleartooddpage{\clearpage\if@twoside
      \ifodd\c@page\else
      \hbox{}\thispagestyle{empty}\newpage
      \if@twocolumn\hbox{}\newpage\fi
      \fi\fi}
    \def\cleartoevenpage{\clearpage\if@twoside
      \ifodd\c@page
      \hbox{}\thispagestyle{empty}\newpage
      \if@twocolumn\hbox{}\newpage\fi
      \fi\fi}
    ...
    % For Polyglossia
    \newfontfamily{\cjkfont}{SourceSerifPro}
    \newfontfamily{\cjkfontsf}{SourceSansPro}
    \newfontfamily{\cjkfonttt}{SourceCodePro}
  \fi
\else
  \ifxetex
    \usepackage{xunicode}
    ...
    % Polyglossia selects these families when entering Japanese.  zxjatype
    % handles Japanese spacing, while these declarations ensure that the
    % language switch does not fall back to the Latin document font.
    \newfontfamily{\cjkfont}[Script=CJK]{Noto Serif CJK JP}
    \newfontfamily{\cjkfontsf}[Script=CJK]{Noto Sans CJK JP}
    \newfontfamily{\cjkfonttt}[Script=CJK]{IPAGothic}

    \newdimen\zw
    \zw=1em
    \newcommand{\kintou}[2]{%
      \leavevmode\hbox to#1{\begingroup
        \xeCJKsetup{CJKglue=\hfill}#2%
      \endgroup}%
    }
    % xeCJKfntef only exposes an ``underdot'' primitive.  A negative depth
    % places the per-character marks above the Japanese text, as kenten.
    \newcommand{\kenten}[1]{\CJKunderdot[depth=-0.9em]{#1}}
    \newcommand{\AttischXeRuby}[2]{%
      \begingroup
        % pxrubrica expects plain text in the ruby annotation.  Preserve the
        % annotation text while discarding an embedded language wrapper.
        \let\textgerman\@firstofone
        \jruby[g]{#1}{#2}%
      \endgroup
    }
    \AtBeginDocument{\let\ruby\AttischXeRuby}
...
\ifdefined\newfontfamily
  ...
  \ifFraktur
    ...
    %\newfontfamily\greekfont[Script=Greek,Ligatures={TeX},
    %ItalicFont = {Old Standard Italic},
    %ItalicFeatures = {LetterSpace=12.0, WordSpace=1.1},
    %]{Old Standard Italic}

    \newfontfamily\greekfont[Script=Greek,Ligatures={TeX},
    ItalicFont = {Theano Old Style},
    ItalicFeatures = {LetterSpace=12.0, WordSpace=1.1},
    ]{Theano Old Style}
    ...
    \ifluatex
      % Request UniFraktur's historical forms explicitly.  LuaTeX-ja needs the
      % post-processing fix loaded below so that its glyph protection does not
      % undo long-s and the ch/tz compositions before letterspacing.
      \newfontfamily\germanfont[
        Renderer=HarfBuzz,
        Script=Latin,
        Language=German,
        Mapping=tex-text,
        Numbers=OldStyle,
        RawFeature={+ccmp,+cv11,+cv13,+cv14,+hlig},
        BoldFont=UnifrakturMaguntia,
        BoldFeatures={Renderer=HarfBuzz,LetterSpace=8.0,Ligatures=NoCommon,Kerning=Off,RawFeature={+ccmp,+cv11,+cv13,+cv14,+hlig,-liga,-kern}},
        ItalicFont=UnifrakturMaguntia,
        ItalicFeatures={Renderer=HarfBuzz,LetterSpace=8.0,Ligatures=NoCommon,Kerning=Off,RawFeature={+ccmp,+cv11,+cv13,+cv14,+hlig,-liga,-kern}}
      ]{UnifrakturMaguntia}
    \else
      \newfontfamily\germanfont[
        Script=Latin,
        ...
        Mapping=tex-text,
        Numbers=OldStyle,
        CharacterVariant={11,13,14},
        Ligatures=Historic,
        BoldFont=UnifrakturMaguntia,
        BoldFeatures={LetterSpace=8.0,Ligatures=NoCommon,Kerning=Off},
        ItalicFont=UnifrakturMaguntia,
        ItalicFeatures={LetterSpace=8.0,Ligatures=NoCommon,Kerning=Off}
      ]{UnifrakturMaguntia}
    \fi
    ...
    \ifluatex
      \AtBeginDocument{\directlua{%
        dofile(assert(kpse.find_file('attisch-luatex-fontfix.lua')))%
      }}
    \fi
...
%
\AddToHook{shipout/background}{\put(1in,-1in){\global\myafterpagetrue}}
...
\ifluatex
  \newcommand{\jdb}[1]{#1\hspace*{-\zw}\rlap{\hspace{0.75\zw}^^^^0323}　}
  \newcommand{\—}{\ltjjachar`—\kern-.5\zw\ltjjachar`—\kern-.5\zw\ltjjachar`—}
\else
  \newcommand{\jdb}[1]{\CJKunderdot{#1}}
  %\newcommand{\—}{—\kern-.5\zw—\kern-.5\zw—}
  \newcommand{\—}{——}
\fi
